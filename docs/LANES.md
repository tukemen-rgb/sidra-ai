...
> **2026-09-16 注記（現行の運用ではない）**: この文書の 6 レーン・Draft PR・ベース
> ブランチ `claude/sidra-ai-gdp-qecrab` の運用は 2026-08-18 で止まっている。
> レーンのブランチは remote に無く、Claude の自動ループは `docs/BACKLOG.md` の
> キューを 1 件ずつ取って **main へ直接 push** している。GDP（ChatGPT）との窓口は
> Issue #372、引き継ぎは `docs/HANDOFF_GDP.md`。本文は経緯の記録として残す
> （他 AI の成果物を理由なく削除しない、の原則）。
...
自分の作業が他レーンの成果に依存する場合、**そのレーンのブランチを merge して
よい**。実際 L3 は L1 を、L6 は L2 を取り込んで進んでおり、これは健全な依存関係
であって違反ではない。

- ✅ 許可: `git merge origin/claude/gdp-l2-security` してその上に自分の担当を積む
- ❌ 禁止: 他レーンの担当ファイルを**自分で書き直す**（同じ機能の二重実装）

判別基準は祖先関係。`git merge-base --is-ancestor <他レーン> <自レーン>` が真なら
取り込み（正常）、偽なのに同じファイルを触っていれば二重実装（要裁定）。

### 赤いブランチを push しない

`pytest` が落ちている状態で push すると、そのレーンを取り込んだ下流のレーンが
まとめて赤くなる。実際に L2 の検知器バグが L6 へ伝播し、両方が赤いまま 9 時間
進んだ。**テストが緑にならないうちは push しない。** 直せないときは変更を
revert し、理由を PR 本文に書く。
