...
> **`--model` の既定値は無くなった**（2026-09-08・C-1160）。以前は `qwen2.5:3b` を既定にしていたが、量子化はカードに合わせて選ぶので**タグは機械ごとに違う**——社長機の実物は `qwen2.5:3b-instruct-q4_K_M` で、既定のまま実行すると「入っていません」で 1 往復した。いまは省略すると、**入っているものが 1 つならそれを使い**、複数なら一覧を出して止まる（勝手に選ばない）。上のように明示して打つのが確実。
...
> **うまくいかないときは、この道具が日本語で理由を言うようになった**（2026-09-08・C-1158）。以前は 15 問ぶん `NG (URLError)` が並ぶだけで、「サーバーが起動していない」「トークンが違う」「モデルを読み込み中」の区別が付かなかった。いまは**同じ原因で残りも失敗する場合はそこで止めて**、何をすればよいかを 1 行で出す。とくに**サーバーは動いているがモデルに繋がっていない**場合は、以前は「日本語率 0%」＝言語の失敗として記録されてしまっていた（実際には何も生成されていない）ので、そこも言い分けるようにした。
> （3B の初回測定がまだなら、載せ替え**前**に 1 回・**後**に 1 回で差が見える）。
> 戻すときは `--model qwen2.5:3b-instruct-q4_K_M` で再実行するだけ。

> **おまけ（1 行・効くのは「その日の最初の 1 問」）**: Ollama は最後の要求から
> **5 分**でモデルを VRAM から降ろすので、しばらく間が空いた後の質問は
> 読み込み直しの時間を丸ごと払う。常駐させたいなら起動前に:
>
> ```powershell
> $env:SIDRA_MODEL_KEEP_ALIVE = "30m"   # "-1" なら降ろさない
> ```
>
> **既定は空＝これまでどおり**（Ollama 自身の 5 分）。常駐させるとその分の
> VRAM を占め続けるので、同じ PC でゲームもするなら `30m` くらいが妥当。
> 効果を確かめたいなら、10 分放置してから同じ質問を 1 回投げて、設定あり／
> なしで体感を比べるのが一番早い（この差はリポジトリ側の計測では見えない
> ——重みを持っている機械でしか起きないため）。
...
> 従来の BM25 単体に戻る設計）。
>
> **プレフィックスが効くことは実測した**（2026-09-08・C-1162）。判定器 38 問で、
> `query: `/`passage: ` を正しく付けた場合 **MRR 0.349**、まったく付けない場合
> **0.307**、両側に同じものを付けた場合 **0.321**。**答えられる問題数は
> 3 通りとも同じ**（18/38）で、変わるのは**正解が何位で返るか**。
> つまりプレフィックスは「付けないと動かない」ものではなく、
> **正解を上に持ち上げる**ものである。模型カードの言い分ではなく
> このコーパスでの数字。
