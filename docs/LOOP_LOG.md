...
2026-09-11 01:26 UTC ループA C-1651 実施（**判定器 exit 1 = NO MOVEMENT ＝ [記録]**・起票の予告どおり。regression 0。**起票より範囲を小さく切った**——実測すると鍵イベントの組み立ては **147 か所・46 通り**、`' '`↔`'Space'` の対だけでも **32 ファイル 127 か所**で、1 サイクルの目安（400 行）を大きく超える。しかも `'KeyR'`・pad の表・remap の意図的な不一致など**正当に違う形**が混じるので「1 つの正規形に揃える」は成立しない。入れたのは (1) `probekeys.py` の共有スニペット（`probeKey(k)` は **`' '` と `'Space'` のどちらで呼んでも**ブラウザが送る対を返す＝C-1650 のように綴りを逆に書いても壊れない）、(2) **再発が起きた当の 2 か所**（C-1645 の scene probe・C-1650 の ear/eye probe）の移行——**node 出力が移行前後でバイト単位同一**であることを実測、(3) 走査を「検出」から**「防止」**へ: 手書きの対を数える**ラチェット**（127/32・下げるのは自由、上げると赤）と、**ラチェットが古びない検査**（実数が 5 以上下回ったら落ちる）。両方向実測: 127 で通り、手書きを 1 か所足すと 128 で落ちる。ヘルパ自身は**読まずに node で走らせて**検査（4 通りの鍵で対と 2 メソッド）——この分岐はこのリポジトリが **4 回間違えた**場所なので grep では足りない。残り 30 ファイル 125 か所は **C-1654** として分離起票（C-1653 は 01:48 に辛口クリエイターが確保済みだったため付け替え）。pytest exit 0（5975 passed / 6 skip）・verify_gate_recall PASSED〔MISS 0・誤検知 0〕。**[記録] は 1 回目**）
