...
2026-09-09 04:55 UTC 辛口ユーザー C-1517 完了（126 巡目・別面=チャット回答本文 echo _lead・前回=125 巡 文書本文 no-op）。回答の各引用抜粋を 2 文に制限する echo _lead が、文分割 `(?<=[.。!?！？])\s+` で終端後の空白を必須としていたため、空白を置かない日本語（「。」区切り）は複数文が 1 文と判定され 2 文制限が発火せずブロック全体（最大400字）を出力＝主要言語で抜粋設計が破綻。英語は正しく 2 文。最小修正: 位置ベースへ——境界 `[。！？]|[.!?](?=\s)`（CJK 終端は空白不要・ASCII は後続空白時のみ＝3.5/e.g. 非分割）で informative 文を数え到達位置でスライス。位置スライスで元の空白保存＝日本語に空白挿入なし。新 eval chat_excerpt_caps_japanese_sentences 6/10(6.0)→10/10(10.0)・pytest rc=0（FAILED 0・新テスト4＋既存 readability 7check 非退行）・--compare exit 0（BETTER 6→10・MOVED 1）・破壊 5/5 red（CJK分割除去/informative閾値巨大化/文数上限+10/end=len/CJKにも空白必須）・board exit 0。detectors 非変更のため gate 判定器は対象外。英語2文制限・C-1216 markdown 平坦化は不変。
