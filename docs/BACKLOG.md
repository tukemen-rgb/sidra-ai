...
- [~] 作業中 2026-08-19 11:48 UTC ループB → 動かす数字: `回答可能率 うち言い換え` 14.3%→（`docs/OUTCOMES.md`）
      **言い換えた質問にほぼ答えられない。**実測で直接語 63.6%(7/11) に対し
      言い換え **14.3%(1/7)** と 4.5 倍の差。BM25 は語の一致で引くので、
      文書の言い回しを知らない人が訊くと当たらない。**実運用で来るのは後者**で、
      「SIDRA に訊けば分かる」と言えない現状の主因はここ。到達率でも
      セキュリティゲートでもない。
      `Retriever` は ABC 化済みなので差し替え口はある（C 節・完了済み）。
      ただし**外部 API と有料依存は不可**、ローカル埋め込みモデルを足すかは
      重い判断になるので、まず軽い手（同義語辞書・クエリ拡張）で
      どこまで戻るかを測ってから決めること。再測定は `measure_outcomes.py`。
