...
/* The music's own dial (§30 事実 3, C-1697). The page has always had one
   ...
   the tune goes and the effects, which are how the player knows what
   just happened, stay. Guarded: a page assembled without the panel has
   no dial to read and plays at full. */
function musicGain(){try{
  const v=tuneNum('music',100);
  return Math.max(0,Math.min(1,(isFinite(v)?v:100)/100))}catch(e){return 1}}
function musicNote(freq,off,dur,vol,wave){
  ...
  /* Zero is silence, not a very quiet tune (§30 事実 3, C-1697) - the same
  ...
  try{
    ...
    const v=Math.min(MAX_GAIN,vol*(COMBAT?COMBAT_GAIN:1))*MUSIC_DUCK*masterGain()*musicGain();
