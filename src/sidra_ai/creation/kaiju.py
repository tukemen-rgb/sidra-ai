...
function draw(){const now=performance.now();
  ...
  cx.fillText(CW.cycles+' '+cycles+'/3  '+CW.legs+' '+Math.max(0,boss.legHp),W-190,5+hudBand(13,1));
  if(state!=='fight'){cx.fillStyle='SCRIM_TOKEN'+'d0';cx.fillRect(0,0,W,H);
    ...
    const a=state==='won'?CW.kaiju_silent:CW.squad_back;announce(a);
    cx.fillText(a,W/2,H/2-8);
    cx.font=hudPx(13)+'px ui-monospace,monospace';
    if((typeof roundAskReady!=='function'||roundAskReady())){const b=CW.again_key;cx.fillText(b,W/2,H/2+18)}cx.textAlign='left'}}
