...
from sidra_ai.creation.fullscreen import (
    ...
    LABEL_ENTER_EN as FULL_LABEL_EN,
...
function step(rt){
  ...
  cx.globalAlpha=HUD_A;cx.fillStyle=HUD_PLATE;
  cx.fillRect(32,14,420,hudBand(16,10));cx.fillRect(32,h-44,330,hudBand(16,10));cx.globalAlpha=1;
  cx.fillStyle=HUD_INK;cx.font=hudPx(16)+'px ui-monospace,monospace';
...
#: Each template's own verb, reached with one thumb (§8 事実 5, C-1702).
#:
...
def _touch_hint(script: str, *, in_japanese: bool = True) -> str:
    ...
    if not groups:
        return (
            "スマホでは画面のボタンで操作できます。"
            if in_japanese
            else "On a phone the on-screen buttons work."
        )
    if in_japanese:
        return "スマホでは画面のボタン（" + " / ".join(groups) + "）で操作できます。"
    return "On a phone the on-screen buttons (" + " / ".join(groups) + ") work."
...
def _page(
    ...
    """C-1956: the page's frame follows the language of the request.

    The frame only - the eight lines a reader sees before pressing a key.
    What the canvas draws (the HUD's 「得点」, the briefing's three lines per
    template, the losing strip) is product copy on the scale of C-1945 and
    is its own item; 「急いで訳すくらいなら訳さないほうがまし」.

    Measured before the split: of 3,076 lines, 184 carry Japanese and 174 of
    them are comments inside ``<script>`` - this repository's design notes,
    which no reader of the page ever sees.
    """
    ...
    # The canvas's fallback content is what a screen reader is handed (§36),
    # so it says the same thing the page says, in the page's language.
    canvas_text = (
        f"{marked_english(title)}——{escape(tagline)} 遊び方: {escape(how)}"
        if in_japanese
        else f"{marked_english(title)} - {escape(tagline)} How to play: {escape(how)}"
    )
    footer_lead = (
        "SIDRA AI が生成。配色と禁止事項の出典:"
        if in_japanese
        else "Generated by SIDRA AI. Sources for the palette and the "
        "prohibitions:"
    )
...
<canvas id="stage" width="720" height="320">{canvas_text}</canvas>
<button class="fullbtn" id="{FULL_BUTTON_ID}" type="button">{escape(FULL_LABEL if in_japanese else FULL_LABEL_EN)}</button>
</div>
<p class="rotatehint" id="{ROTATE_ID}">{escape(ROTATE_TEXT if in_japanese else ROTATE_TEXT_EN)}</p>
<p class="how">{escape(how)}</p>
<p class="touchhint">{_touch_hint(script, in_japanese=in_japanese)}</p>
...
def generate_game(
    ...
    *,
    ...
    script = with_animation(
        ...
        # The floor an operator-chosen accent is held to (C-1737). Taken
    ...
    # Imported here rather than at module scope: `creation.__init__` imports
    ...
    # table, and the difficulty word is already the English one the request
    # used ("normal", "hard").
    if _reply_in_japanese(request):
        tagline = f"難易度 {difficulty} / ジャンル {genre_label}"
    else:
        tagline = f"difficulty {difficulty} / kind {english_label_for(key)}"
    asked_title = title
    ...
    if named:
        ...
        tagline = (
            "依頼にあった作品名は使えないためオリジナル版 / "
            if _reply_in_japanese(request)
            else "an original, because the name in the request cannot be used / "
        ) + tagline
    ...
    html = _page(
        title,
        tagline,
        spec.how_to_play if in_japanese else spec.how_to_play_en,
        script,
        list(evidence or [_SOURCE]),
        theme,
        note=fallback_note,
