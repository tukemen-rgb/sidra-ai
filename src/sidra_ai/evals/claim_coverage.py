...
# Only tightly scoped continuations that keep the sentence in an abstention/meta
# posture are exempt from claim-local citation requirements. A substantive tail
# such as "No indexed evidence, the API is public by default" must not inherit
# non-claim status merely from its prefix.
_ABSTENTION_BENIGN_TAIL_PREFIXES = (
    "matched ",
    "matches ",
    "matching ",
    "to ",
...
def _is_nonclaim(sentence: str) -> bool:
    ...
    if any(normalized.startswith(prefix) for prefix in _NONCLAIM_PREFIXES):
        return True
    ...
    for prefix in _ABSTENTION_PREFIXES:
        ...
        if not tail:
            return True
        return any(
            tail.startswith(benign)
            for benign in _ABSTENTION_BENIGN_TAIL_PREFIXES
        )
...
def run_claim_citation_coverage_suite() -> tuple[EvalOutcome, ...]:
    ...
    return (
        ...
        EvalOutcome(
            ...
            detail=(
                "an abstention prefix must not exempt a substantive uncited tail; "
                "benign no-evidence wording remains allowed"
            ),
            failures=(
                ()
