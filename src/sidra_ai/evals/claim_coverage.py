...
# These are non-claim/status lines that can legitimately appear without a
# source citation. Keep this list deliberately narrow: broad exemptions would
# recreate the bypass this evaluator is meant to catch.
_NONCLAIM_PREFIXES = (
    "question received:",
    "cited sources:",
    "sources:",
...
# Support the common ``claim. [S1]`` form by moving citation-only suffixes
# inside the sentence before splitting. This is normalization for evaluation
# only; the original answer is never modified or returned.
...
def evaluate_claim_citation_coverage(
    ...
    This evaluator is complementary to ``evaluate_grounding``:
    - if no retrieved labels exist, no work is done because the no-evidence
      abstention evaluator owns that case;
    - if the answer uses no valid citation at all, no work is done because the
      answer-level grounding evaluator already rejects it;
    - once at least one valid citation is used, every other factual sentence
      must also contain a valid retrieved citation.

    This catches answers such as ``"GitHub is read-only [S1]. The API is public
    by default."`` where answer-level citation presence alone would otherwise
    make the response look grounded.
    """
