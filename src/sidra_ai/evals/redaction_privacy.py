...
def _overlapping_pii_case() -> EvalOutcome:
    personal = "alice@example.test"
    result, serialized = _inspect_with_audit(f"password={personal}")
    failures: list[str] = []

    if result.decision is not Decision.QUARANTINE:
        failures.append(f"expected quarantine, got {result.decision.value}")
    if personal in serialized:
        failures.append("overlapping personal identifier survived into an audit surface")
    if fingerprint(personal) in serialized:
        failures.append("overlapping PII inherited a deterministic secret fingerprint")
    if result.content != "password=[REDACTED:pii_email]":
        failures.append("overlapping secret/PII span did not persist as a PII placeholder")

    return EvalOutcome(
        case_name="redaction_overlapping_pii_has_no_fingerprint",
        passed=not failures,
        detail=result.decision.value,
        failures=tuple(failures),
    )
