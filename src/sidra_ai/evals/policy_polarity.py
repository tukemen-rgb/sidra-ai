...
reverse the meaning of cited evidence without inventing any literal. This module
covers a small set of high-impact security/cost/verification invariants without
a paid judge model.
"""
...
def evaluate_policy_polarity(answer: str, evidence_by_label: Mapping[str, str]) -> PolicyPolarityResult:
    failures: list[str] = []
    checked = 0
    for claim in _claim_units(answer):
        ...
        evidence = "\n".join(
            ...
            for label in labels if label in evidence_by_label
        )
        if not evidence:
            continue
        for rule in _RULES:
            ...
            if claim_polarity != evidence_polarity:
                failures.append(f"{rule.name} policy/status polarity contradicts locally cited evidence")
    return PolicyPolarityResult(not failures, checked, tuple(failures))
...
def run_policy_polarity_suite() -> tuple[EvalOutcome, ...]:
    ...
    checks = (
        ("supported read-only policy was rejected", evaluate_policy_polarity("GitHub write capability is disabled. [S1]", evidence), True),
        ("reversed GitHub write policy escaped citation-local polarity check", evaluate_policy_polarity("GitHub write capability is enabled. [S1]", evidence), False),
        ("reversed public-bind policy escaped citation-local polarity check", evaluate_policy_polarity("Public binding is allowed by default. [S2]", evidence), False),
        ("reversed paid-API policy escaped citation-local polarity check", evaluate_policy_polarity("External paid LLM API is required. [S3]", evidence), False),
        ("Japanese reversed GitHub write policy escaped polarity check", evaluate_policy_polarity("GitHub書き込みは可能です。[S1]", evidence), False),
        ("supported unverified-test status was rejected", evaluate_policy_polarity("Full pytest has not been verified green. [S4]", evidence), True),
        ("false test-green claim escaped citation-local status check", evaluate_policy_polarity("Full pytest passed successfully. [S4]", evidence), False),
        ("Japanese false test-green claim escaped status check", evaluate_policy_polarity("全pytestは通過済みです。[S4]", evidence), False),
    )
    failures = [message for message, result, expected_pass in checks if result.passed is not expected_pass]
    return (
        EvalOutcome(
            "rag_policy_and_verification_polarity_support",
            not failures,
            "security/cost policy and verification-status claims must not reverse locally cited evidence",
            tuple(failures),
        ),
    )
