...
def _sanitize_finding_evidence(evidence: str) -> str:
    ...
    Detectors are deliberately independent and future detectors may
    accidentally hand :class:`Finding` a raw matching snippet.  The audit
    model is the final trust boundary before findings can reach quarantine or
    API audit records, so it enforces the documented invariant centrally
    rather than relying on every detector author to remember it.

    Existing ``excerpt()`` placeholders are preserved so their span length is
    not replaced by the length of the placeholder itself.
    """
