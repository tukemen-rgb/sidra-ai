...
def _expected_context_free_provenance(provenance: Provenance) -> dict[str, object]:
    return {
...
def test_rejected_document_provenance_is_context_free_in_quarantine(tmp_path) -> None:
    ...
    assert audit_provenance == _expected_context_free_provenance(provenance)
...
def test_oversized_block_provenance_is_context_free_before_secret_pii_scan(tmp_path) -> None:
    ...
    provenance = Provenance(
        ...
        url=f"https://example.invalid/{synthetic_secret}",
        author=synthetic_pii,
        extra={"secret": synthetic_secret, "pii": synthetic_pii},
    )
    store = QuarantineStore(tmp_path / "quarantine.jsonl")
    gate = SecurityGate(
    ...
    result, screened = gate.screen_document(
        Document(content="oversized payload", provenance=provenance)
    )
    ...
    assert entry["provenance"] == _expected_context_free_provenance(provenance)

    serialized = json.dumps(entry, ensure_ascii=False)
    for sensitive in (synthetic_secret, synthetic_pii):
        assert sensitive not in serialized
    for raw_extra_key, raw_extra_value in provenance.extra.items():
        assert raw_extra_key not in serialized
        assert raw_extra_value not in serialized
