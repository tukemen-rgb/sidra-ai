...
def test_source_heading_does_not_launder_uncited_claim() -> None:
    result = evaluate_claim_citation_coverage(
...
def test_provenance_phrase_does_not_launder_uncited_claim() -> None:
    result = evaluate_claim_citation_coverage(
        "GitHub access is read-only [S1]. "
...
def test_exact_metadata_headings_remain_nonclaims() -> None:
    source_heading = evaluate_claim_citation_coverage(
        "GitHub access is read-only [S1]. Sources: [S1] [S2]",
        _CITATIONS,
    )
    provenance_heading = evaluate_claim_citation_coverage(
        "GitHub access is read-only [S1]. Answering from indexed repository data.",
        _CITATIONS,
    )

    assert source_heading.passed is True, source_heading.failures
    assert provenance_heading.passed is True, provenance_heading.failures


def test_release_suite_contains_abstention_and_metadata_prefix_guards() -> None:
    outcomes = run_claim_citation_coverage_suite()
    by_name = {outcome.case_name: outcome for outcome in outcomes}

    assert by_name["rag_abstention_prefix_claim_laundering_rejected"].passed is True
    assert by_name["rag_nonclaim_prefix_claim_laundering_rejected"].passed is True
