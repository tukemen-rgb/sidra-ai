...
def test_output_guard_blocks_hex_encoded_credential() -> None:
    """Hex must not hide a provider token from the output guard."""

    from sidra_ai.security.output_guard import OutputGuard

    synthetic_secret = "ghp_" + "0" * 36
    encoded = synthetic_secret.encode().hex()
    result = OutputGuard().scan(f"Hex credential: {encoded}")
...
def test_output_guard_blocks_hex_encoded_personal_email() -> None:
    from sidra_ai.security.output_guard import OutputGuard

    personal = "person@example.invalid"
    encoded = personal.encode().hex()
    result = OutputGuard().scan(f"Hex contact: {encoded}")
...
def test_output_guard_allows_safe_hex_text() -> None:
    from sidra_ai.security.output_guard import OutputGuard

    encoded = "SIDRA AI local model".encode().hex()
    text = f"Hex note: {encoded}"
    result = OutputGuard().scan(text)
